...
// Неймспейс по умолчанию — используется, когда компонент вызывает t() без явного указания неймспейса через useTranslation('ns').
// ВАЖНО: этот же экспорт использует i18next.d.ts для типизации CustomTypeOptions.defaultNS — без него компиляция падает с ошибкой "no exported member 'defaultNS'"
